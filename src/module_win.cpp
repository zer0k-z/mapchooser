...
void *CModule::FindVirtualTable(const std::string &name)
{
	...
	const uint32_t rttiTDRva = (uint32_t)((uintptr_t)typeDescriptor - (uintptr_t)m_base);
	...
	{
		uintptr_t completeObjectLocatorHeader = (uintptr_t)completeObjectLocator - 0xC;

		// Check RTTI Complete Object Locator signature, always 0x1 for 64-bit
		if (*(int32_t *)(completeObjectLocatorHeader) != 1)
			continue;

		// Check vtable offset, must be 0 for the primary vtable
